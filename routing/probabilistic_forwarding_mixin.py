...
class ProbabilisticForwardingMixin(object):
    ...
    def __get_additional_peers(self, channel, ignore_peers=None):
        ...
        # get a set of all peers we are still connected to that are not in our ignore_peers iterable
        ...
        # this are the peers selected for probabilistic forwarding we are still connected to (with to_removed ones removed)
        self.__probabilistic_forwarding_peers[channel] &= connections
        # this is the list of connections that can be used to select (additional) peers for probabilistic forwarding from
        ...
        elif size < len(self.__probabilistic_forwarding_peers[channel]):
            logger.warning("Having %d more probabilistic forwarding peers for channel '%s' than the %d wanted ones!" % (
                len(self.__probabilistic_forwarding_peers[channel]) - size,
                str(channel),
                size
            ))
